...
<Steps>
  ...
  <Step title="Prepare PostgreSQL and Redis">
    ...
    <Note>
      The auth service runs its own migrations on startup. The bot connects to PostgreSQL and expects the schema to already be in place. The bot also needs Redis before `cargo run` succeeds.
    </Note>
  </Step>
