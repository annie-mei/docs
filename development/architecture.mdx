...
migrations/          # Historical bot migrations for the removed legacy users table
```
...
Annie Mei uses [SQLx](https://github.com/launchbadge/sqlx) for PostgreSQL queries. The bot shares a database with the auth service and reads from the `oauth_credentials` table directly via async SQL queries.

<Note>
  The bot previously used Diesel ORM. As of v2.15.0, the bot has no Diesel-managed tables. The current OAuth schema migrations live in the companion auth service repo and run during auth-service startup. The bot repo's legacy `users` table was dropped in migration `2026-05-02-000001_drop_users_table`.
</Note>
...
3. **Command Routing** (`main.rs`)
   ...
   - Commands: `ping`, `help`, `songs`, `manga`, `anime`, `search`, `recommend`, `character`, `register`, `unregister`, `whoami`
