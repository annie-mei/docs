...
<Steps>
  ...
  <Step title="Add Match Arm in interaction_create">
    ...
    match command.data.name.as_str() {
        ...
        "example" => commands::example::run(&ctx, &mut command).await,  // Add this line
        _ => {
            // Error handler...
        }
    };
...
pub async fn run(ctx: &Context, interaction: &mut CommandInteraction) {
    ...
    // Long operation (e.g., async API call or database query)
    let result = fetch_example_data().await;
...
Use `spawn_blocking` only for synchronous work that can block the Tokio runtime, such as the current Redis cache helper calls.
