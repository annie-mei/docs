...
- **SQLx CLI** (optional) - For running or creating migrations manually during development
...
<Steps>
  ...
  <Step title="Run Migrations">
    Apply all database migrations using SQLx if you want to prepare the database before starting the bot:

    ```bash
    sqlx migrate run
    ```

    Annie Mei also runs embedded migrations automatically on startup, so this step is optional for a normal local run. See the [Database](/development/database) guide for more details.
  </Step>
