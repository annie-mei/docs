...
The bot service does not run migrations on startup and has no bot-owned tables. It connects to a database that already contains the auth-service-owned OAuth schema.
...
<Warning>
  Do not use the bot repo's historical migrations to prepare the OAuth schema. They only document the removed legacy `users` table and do not create `oauth_credentials` or `oauth_sessions`.
</Warning>

### Bot repo historical migrations
...
<AccordionGroup>
  ...
  <Accordion title="Migration failures">
    Check the auth service migration state from the auth repo:
