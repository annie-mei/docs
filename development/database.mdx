...
The bot still connects to a database that already contains the auth-service-owned `annie_auth` schema.

The current OAuth schema migrations live in the companion auth service repo under `migrations/`. On startup, the auth service creates `annie_auth`, opens a migration connection with `search_path=annie_auth,public`, and runs `sqlx::migrate!("./migrations")`. Its migration history therefore lives in `annie_auth._sqlx_migrations`.

<Warning>
  Do not use the bot repo's migrations to prepare the OAuth schema. The bot migrations only create bot-owned settings tables and do not create `annie_auth.oauth_credentials` or `annie_auth.oauth_sessions`.
</Warning>
...
The auth service runs migrations during startup. Starting the service is the preferred way to apply them:
...
If you use SQLx CLI directly, set the auth migration search path so it reads and writes the same migration history as service startup:

```bash
cd ../auth
PGOPTIONS='-c search_path=annie_auth,public' sqlx migrate run
PGOPTIONS='-c search_path=annie_auth,public' sqlx migrate info
```

## Querying with SQLx

The bot reads from the auth-service-owned `annie_auth.oauth_credentials` table using `sqlx::query_as` with `FromRow` structs:
...
pub async fn get_by_discord_ids(
    ...
    sqlx::query_as::<_, OAuthCredential>(
        "SELECT discord_user_id, anilist_id, anilist_username \
         FROM annie_auth.oauth_credentials WHERE discord_user_id = ANY($1)"
    )
    .bind(ids)
    .fetch_all(pool)
    .await
...
<AccordionGroup>
  ...
  <Accordion title="Migration failures">
    ...
    For persistent auth issues, inspect `annie_auth._sqlx_migrations` directly.
  </Accordion>
