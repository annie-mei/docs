...
test('buildDocumentationPrompt contains only deterministic release context', () => {
  ...
  expect(prompt).toContain('annie-211-docs-3-10-0')
  expect(prompt).toContain('[ANNIE-211]/Update docs for Annie Mei v3.10.0')
  expect(prompt).toContain('wait for automated reviews and required checks')
  expect(prompt).toContain("merge using the repository's normal merge method")
  expect(prompt).toContain('Do not bypass protections')
})
