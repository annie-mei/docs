...
| Auth service | `auth/` | Validates signed OAuth context, redirects to AniList, handles callback, stores OAuth credentials in `annie_auth`, and serves stack health/readiness endpoints | AniList OAuth, PostgreSQL |
...
`/healthz` returns liveness with `services.database` set to `not_checked`. `/readyz` checks PostgreSQL with a two-second timeout and reports `up` or `down`.
