...
For complete self-hosted deployments, plan for two processes: the Discord bot and the AniList OAuth auth service. The bot needs PostgreSQL and Redis. The auth service needs PostgreSQL.

The hosted Annie Mei bot currently uses Supabase, Upstash, and Doppler. Those providers are not self-hosting requirements; you can use any compatible PostgreSQL, Redis, secrets workflow, and compute provider.
...
4. **Start the auth service, then the bot**. Each service creates its schema and runs its own SQLx migrations during startup.
...
The auth service owns `annie_auth.oauth_credentials`, `annie_auth.oauth_sessions`, and `annie_auth._sqlx_migrations`. Its schema is defined in:
- `auth/migrations/` (SQLx migration files in the auth service repo)

Auth startup creates `annie_auth`, runs migrations with `search_path=annie_auth,public`, and then closes its dedicated migration pool. The bot repo's `migrations/` directory contains bot-owned migrations and migration history in the `annie_mei` schema. It is not used to prepare the OAuth schema.

The bot reads from the auth-service-owned `annie_auth.oauth_credentials` table. The database stores:
...
The bot reuses Redis connections through a pool capped at eight connections. It bounds pool waits and Redis connection, read, and write operations to two seconds; I/O-failed connections are discarded.
...
The auth service's Railway configuration uses `/readyz` for deployment health checks. It binds to the platform-provided `$PORT`, allows 60 seconds for the health check, and uses an on-failure restart policy with at most five retries.
