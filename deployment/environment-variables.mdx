...
<Warning>
  The bot and auth service must point at the same PostgreSQL database. The auth service owns the OAuth tables, and the bot reads `oauth_credentials` from that shared database for `/whoami`, `/unregister`, and member status overlays.
</Warning>
