...
The auth service creates `annie_auth` and runs its OAuth schema migrations on startup with migration history in `annie_auth._sqlx_migrations`. The bot creates `annie_mei` and runs its settings migrations on startup with separate migration history. Start the auth service before the bot when deploying a fresh shared database.
...
`/healthz` uses the same shape with `"database": "not_checked"`. An unavailable or timed-out readiness database check returns `503` with `"status": "unhealthy"` and `"database": "down"`.
