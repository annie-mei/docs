...
<ResponseField name="Thumbnail">
  Anime cover image from MyAnimeList, when available. Missing image data does not prevent the song list from appearing.
</ResponseField>
...
<Steps>
  ...
  <Step title="Match with Spotify">
    The bot checks cached matches, then reuses one Spotify login while it searches the opening and ending songs in the response.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Missing Spotify links">
    Not all songs have Spotify matches. A temporary Spotify authentication or search failure can also leave a link unavailable. The bot will still show:
    - Song title
    - Artist name
    - Episode range
...
<Note>
  ...
  - Malformed or missing MyAnimeList image and episode-range fields are omitted instead of failing the whole response
</Note>

## Source Code Reference

Implementation: `src/commands/songs/command.rs`
...
The command fetches data through `src/commands/songs/fetcher.rs`, parses MyAnimeList data in `src/models/mal_response.rs`, and enriches the combined opening and ending list through `src/utils/spotify.rs`.
