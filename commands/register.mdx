...
3. The button opens `${AUTH_SERVICE_BASE_URL}/oauth/anilist/start` in your browser.
4. After AniList authorization, the bot stores your AniList identity.
...
<AccordionGroup>
  ...
  <Accordion title="I want to unlink my AniList account">
    The bot has no `/unregister` command. Contact your bot administrator to remove stored link data.
  </Accordion>
</AccordionGroup>
...
`register()` defines the slash command and `run()` creates the OAuth link for `AUTH_SERVICE_BASE_URL`.

The final link handling happens in the auth service repository (`../auth`).
