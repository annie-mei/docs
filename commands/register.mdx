...
The auth service also stores AniList OAuth credentials (access and refresh tokens) and short-lived OAuth session state needed to complete and maintain the link. The bot does not store your AniList username — `/whoami` displays your numeric AniList ID instead.
